...
import { fixTailwindColors, cleanupColorConverter } from '../utils/colors'
...
const EditResume = () => {
  ...
  const goToNextStep = () => {
    ...
    //back Previous form state or previous page
  const goBack = () => {
    const pages = [
      "profile-info",
      "contact-info",
...
//fetch resume details by ID(backend url)
  ...
  //chooses preview and download resume
...
//updates the resume, put will help in updation in backend
...
// Make sure you have imported jsPDF and html2canvas
// import { jsPDF } from "jspdf";
// import html2canvas from "html2canvas";

const downloadPDF = async () => {
    const element = resumeDownloadRef.current; // The original element
    if (!element) {
        toast.error("Failed to generate PDF. Please try again.");
        return;
    }
    ...
    let colorFixStyle = null; // For <a> tags
    const tempId = `h2c-temp-id-${Date.now()}`; // Unique ID for finding the clone

    try {
        // --- FIX FOR BLUE <a> LINK PROBLEM ---
        // This is still needed for the html2canvas link bug
        colorFixStyle = document.createElement('style');
        colorFixStyle.id = '__h2c_link_color_fix__';
        colorFixStyle.innerHTML = `
          .${element.classList[0]} a,  
          .${element.classList[0]} a:link,  
          .${element.classList[0]} a:visited {
        ...
        // 1. Add a temporary data-attribute to the *original* element
        //    so we can find its clone inside the onClone handler.
        element.dataset.h2cTarget = tempId;

        // 2. Define the onClone handler
        //    This function will run *after* html2canvas clones the document
        //    but *before* it renders it to the canvas.
        const onCloneHandler = (clonedDoc) => {
            // Find the cloned version of our resume element
            const clonedElement = clonedDoc.querySelector(`[data-h2c-target="${tempId}"]`);
            if (!clonedElement) {
                console.error("Could not find cloned element for fixing.");
                return;
            }

            // --- Start of oklch fix logic (formerly in colors.js) ---
            
            // A. Create the color converter utility (inside the *cloned* doc)
            ...
            const convertOklchValue = (oklchColor) => {
                try {
                    converter.style.color = oklchColor;
                    const computedStyle = window.getComputedStyle(converter);
                    return computedStyle.color; // Returns 'rgb(...)'
                } catch (e) {
            ...
            const convertOklchStrings = (cssString) => {
                ...
                return cssString.replace(oklchRegex, (match) => convertOklchValue(match));
            };

            const COLOR_PROPERTIES = [
                'color', 'backgroundColor', 'borderColor', 'borderTopColor', 'borderRightColor',
                'borderBottomColor', 'borderLeftColor', 'outlineColor', 'textDecorationColor',
                'columnRuleColor', 'boxShadow', 'textShadow', 'fill', 'stroke', 'filter'
            ];

            // B. Process all elements in the clone
            const allElements = [clonedElement, ...clonedElement.querySelectorAll('*')];
            let pseudoElementStyles = [];

            allElements.forEach((el, index) => {
                const computed = window.getComputedStyle(el);
                el.dataset.h2cId = index; // Add ID for pseudo-element targeting

                // Process main element properties
                COLOR_PROPERTIES.forEach(prop => {
                    const value = computed[prop];
                    if (value && typeof value === 'string') {
                        el.style[prop] = convertOklchStrings(value);
                    }
                });

                // C. Process pseudo-elements
                const beforeComputed = window.getComputedStyle(el, '::before');
                const afterComputed = window.getComputedStyle(el, '::after');
                const uniqueSelector = `[data-h2c-target="${tempId}"] [data-h2c-id="${index}"]`;
                ...
                const buildPseudoRules = (styles) => {
                    ...
                    COLOR_PROPERTIES.forEach(prop => {
                        ...
                        if (value && typeof value === 'string') {
                            rules.push(`${prop}: ${convertOklchStrings(value)} !important`);
                        }
                    });
                    return rules.join('; ');
                };
                ...
                if (beforeComputed.content && beforeComputed.content !== 'none') {
                    pseudoElementStyles.push(`${uniqueSelector}::before { ${buildPseudoRules(beforeComputed)} }`);
                }
                if (afterComputed.content && afterComputed.content !== 'none') {
                    pseudoElementStyles.push(`${uniqueSelector}::after { ${buildPseudoRules(afterComputed)} }`);
                }
            });

            // D. Inject pseudo-element styles into the cloned doc
            if (pseudoElementStyles.length > 0) {
                const styleTag = clonedDoc.createElement('style');
                styleTag.id = '__h2c_pseudo_fix__';
                styleTag.innerHTML = pseudoElementStyles.join('\n');
                clonedDoc.head.appendChild(styleTag);
            }
            
            // E. Clean up the converter div
            clonedDoc.body.removeChild(converter);
            // --- End of oklch fix logic ---
        };

        // 3. Generate Canvas from the *original* element
        //    Pass our onClone handler in the options
        const canvas = await html2canvas(element, { 
            scale: 2, 
            useCORS: true, 
            backgroundColor: "#FFFFFF",
            onclone: onCloneHandler // <-- This is the new, important part
        });
        ...
        // --- FIX FOR LARGE TEXT / 2-PAGE PROBLEM ---
        // (This is the correct multi-page logic from our previous conversation)
        ...
        // 7. IMPORTANT: Cleanup is now much simpler
        
        // Remove the temporary ID from the *original* element
        if (element) {
            delete element.dataset.h2cTarget;
        }
        
        // Remove the <a> tag fix style
        if (colorFixStyle && document.head.contains(colorFixStyle)) {
            document.head.removeChild(colorFixStyle);
        }
        
        // All other DOM changes were in the clone, which is now gone.
        // No need to clean up pseudoFixStyleTag, fixedElement, or converter.
        ...
        <div className={containerStyles.main}>
            <div className={containerStyles.header}>
                ...
                <div className='flex flex-wrap items-center gap-3'>
                    ...
                    <button onClick={handleDeleteResume} className={buttonStyles.delete} delete={isLoading}>
                        <Trash2 size={16}/>
                ...
                <div className={containerStyles.previewContainer}>
                  <div className="text-center mb-4">
                    <div className={statusStyles.completionBadge}>
                      ...
                      <span>
                        Preview - {completionPercentage}% Completed
                      </span>
                    </div>
...
import StepProgress from './StepProgress'
import { jsPDF } from 'jspdf';
// Find the correct path in your node_modules for the browser version
import html2pdf from 'html2pdf';
// OR, if the package is just 'html2pdf':
